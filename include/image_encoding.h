...
inline sf::Texture gray_scale(sf::Texture texture) {
    // Récupérer l'image à partir de la texture
    sf::Image image = texture.copyToImage();
    sf::Vector2u size = image.getSize();

    // Récupérer le tableau de pixels de l'image (taille = width * height * 4)
    ...
    for (unsigned int y = 0; y < size.y; ++y) {
        for (unsigned int x = 0; x < size.x; ++x) {
            ...
            // Calcul du niveau de gris selon la pondération donnée
    ...
    // Créer une texture et la charger avec la nouvelle image
